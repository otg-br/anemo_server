...
keywordHandler:addKeyword({'captain'}, StdModule.say, {npcHandler = npcHandler, text = 'I am the captain of Mordragor, medieval city where was forged after killing an ancient evil.'})
keywordHandler:addKeyword({'sail'}, StdModule.say, {npcHandler = npcHandler, text = 'Where do you want to go? To {Falanaar}, {Dolwatha}, {Death Valley}, {Freewind} or {Arkeron}?'})


local node1 = keywordHandler:addKeyword({'falanaar'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you seek a passage to Falanaar for 120 gold?'})
    node1:addChildKeyword({'yes'}, StdModule.travel, {npcHandler = npcHandler, premium = true, cost = 120, destination = Position(712, 461, 6)})
...
local node2 = keywordHandler:addKeyword({'dolwatha'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you seek a passage to Dolwatha for 160 gold?'})
    node2:addChildKeyword({'yes'}, StdModule.travel, {npcHandler = npcHandler, premium = true, cost = 160, destination = Position(510, 318, 6)})
...
local node3 = keywordHandler:addKeyword({'death valley'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you seek a passage to Death Valley for 250 gold?'})
    node3:addChildKeyword({'yes'}, StdModule.travel, {npcHandler = npcHandler, premium = true, cost = 250, destination = Position(1318, 645, 7)})
