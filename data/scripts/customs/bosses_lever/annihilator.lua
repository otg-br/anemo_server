local config = {
    actionId = 43017, 
    bossName = "Demon",
    bossPosition = Position(1060, 517, 13), 
    bossArea = {
        fromPos = Position(1056, 512, 13), 
        toPos = Position(1084, 523, 13), 
        entrancePos = Position(1059, 519, 13), 
        exitPosition = Position(1074, 532, 13) 
    },
    participantsAllowAnyCount = true, 
    participantsPos = {
        Position(1075, 533, 13),
		Position(1074, 533, 13),
		Position(1073, 533, 13),
		Position(1072, 533, 13)
    },
    attempts = {
        level = 100, 
        storage = 710013, 
    ...
    createTeleportPos = Position(1083, 517, 13), 
    teleportToPosition = Position(1073, 532, 13), 
    teleportRemoveSeconds = 120, 
    kickParticipantAfterSeconds = 120 * 10,
    leverIds = {1945, 1946},
    demonPositions = {
        Position(1060, 517, 13),
        Position(1062, 517, 13),
        Position(1064, 517, 13),
        Position(1066, 517, 13),
        Position(1060, 521, 13),
        Position(1062, 521, 13),
        Position(1064, 521, 13),
        Position(1066, 521, 13),
        Position(1065, 519, 13),
        Position(1066, 519, 13)
    }
}
...
function annihilator.onUse(player, item, fromPos, target, toPos, isHotkey)
    local participants = {}
    for index, pos in pairs(config.participantsPos) do
        ...
        if participant and participant:isPlayer() then
            ...
            if participant:getStorageValue(config.attempts.storage) >= os.time() then
                player:sendCancelMessage(string.format("The player %s must wait 100 seconds before being able to enter again.", participant:getName()))
                return true
    ...
    boss:registerEvent("annihilatorDemons")
    for i = 1, #config.demonPositions do
        local demonPos = config.demonPositions[i]
        local newDemon = Game.createMonster(config.bossName, demonPos)
        if newDemon then
            spectators[#spectators + 1] = newDemon
        end
    end
    ...
    config.kickEventId = addEvent(function ()
        for _, spectator in pairs(getSpectators()) do
            if spectator:isPlayer() then
                ...
                spectator:sendTextMessage(MESSAGE_INFO_DESCR, "It's been a long time and you haven't managed to defeat the Demons.")
            else
                spectator:remove()
            end
        end
...
annihilator:aid(config.actionId)
annihilator:register()

local creatureEvent = CreatureEvent("annihilatorDemons")
